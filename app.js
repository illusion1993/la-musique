...
function searchInStationsGenre(keyword) {
	console.log("Search genre initiated___");
	var results = [];
	for (station_index in ALL_STATIONS) {
		var cur = ALL_STATIONS[station_index];
		if (cur.genre && cur.genre.indexOf(keyword) != -1) {
			results.push(cur);
		}
	}
	console.log("Search genre finished___");
