module.exports = function () {
	...
	Node.prototype.insert = function(word, data, index, type) {
		this.data[data] = true;
		if (type) this.data_type = type;

		if (index < word.length) {
			var next_character = word[index];
			if (!this[next_character]) this[next_character] = new Node(next_character).index;
			NODE_LIST[this[next_character]].insert(word, data, index + 1, type);
		}
	};
	...
	Trie.prototype.insert = function(word, data, type) {
		if (word) this.root.insert(word, data, 0, type);
	...
	Trie.prototype.get_nodes = function() {
		NODE_LIST.forEach(function(obj, index, arr) {
			arr[index].data = Object.keys(arr[index].data);
		});
		return NODE_LIST;
	};
